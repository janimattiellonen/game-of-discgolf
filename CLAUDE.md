...
- `pnpm check` — typecheck, then oxlint, then `prettier --check`. One command,
  fail-fast, and it is the same one CI would run.
...
- **Run it.** There is no test suite — no runner, no `pnpm test`, nothing to hide
  behind. `pnpm dev` and throw a few discs is the only evidence that a change to
  `physics.ts`, `sim.ts` or `constants.ts` did what you meant. Green types prove
  the code compiles, not that the disc flies.
